...
#ifndef _MultipleToSerial_tmp_elements_RAM_AUTO_1R1W_H_
#define _MultipleToSerial_tmp_elements_RAM_AUTO_1R1W_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "MultipleToSerial_tmp_elements_RAM_AUTO_1R1W_memcore.h"

SC_MODULE(MultipleToSerial_tmp_elements_RAM_AUTO_1R1W) {
...
protected:
    ...
    // buffer instances
    MultipleToSerial_tmp_elements_RAM_AUTO_1R1W_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(MultipleToSerial_tmp_elements_RAM_AUTO_1R1W) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "MultipleToSerial_tmp_elements_RAM_AUTO_1R1W_memcore_U_";
            name += szTmp;

            buffer[i] = new MultipleToSerial_tmp_elements_RAM_AUTO_1R1W_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "MultipleToSerial_tmp_elements_RAM_AUTO_1R1W.trace");
            // ports
